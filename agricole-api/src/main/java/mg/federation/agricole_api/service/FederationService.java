...
public class FederationService {
    ...
    public Member createMember(CreateMemberDTO dto) {
        ...
        if (localReferees < (referees.size() - localReferees) || referees.size() < 2) {
    ...
    public Collectivity createCollectivity(CreateCollectivityDTO dto) {
        ...
        if (dto.getMembers() == null || dto.getMembers().size() < 10) {
            throw new BadRequestException("Une collectivité doit avoir au moins 10 membres");
        }

        List<Member> members = memberRepo.findAllById(dto.getMembers());
        if (members.size() != dto.getMembers().size()) {
            throw new NotFoundException("Un ou plusieurs membres sont introuvables");
        }

        CollectivityStructure struct = new CollectivityStructure();
        struct.setPresident(memberRepo.findById(dto.getStructure().getPresident())
                .orElseThrow(() -> new NotFoundException("Président introuvable")));
        struct.setVicePresident(memberRepo.findById(dto.getStructure().getVicePresident())
                .orElseThrow(() -> new NotFoundException("Vice-Président introuvable")));
        struct.setTreasurer(memberRepo.findById(dto.getStructure().getTreasurer())
                .orElseThrow(() -> new NotFoundException("Trésorier introuvable")));
        struct.setSecretary(memberRepo.findById(dto.getStructure().getSecretary())
                .orElseThrow(() -> new NotFoundException("Secrétaire introuvable")));
    ...
    @Transactional
    public Collectivity identifyCollectivity(String id, String name, Integer number) {
        ...
        if (col.getName() != null || col.getNumber() != null) {
            throw new BadRequestException("Nom et numéro déjà attribués");
        }
